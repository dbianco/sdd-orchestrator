...
describe.skipIf(!url)('routeTask and startFeature', () => {
  ...
  it('fires overBudgetPack for a lite pack whose always-on content alone exceeds the app budget', async () => {
    ...
    const metrics = { routed: vi.fn(), gate: vi.fn(), degradedPack: vi.fn(), overBudgetPack: vi.fn(), failedCycle: vi.fn(), authRejected: vi.fn(), approval: vi.fn(), ciEvidence: vi.fn(), requirementsUncovered: vi.fn() };
