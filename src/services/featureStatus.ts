...
import { requirementStatus, type RequirementStatus } from './requirements.js';
...
export interface FeatureStatusResult extends FeatureState { transitions: TransitionSummary[]; latest_pack_per_phase: Record<string, string>; requirements: RequirementStatus[] }
...
export async function getFeatureStatus(deps: ServiceDeps, featureId: string): Promise<FeatureStatusResult> {
  ...
  return { ...state, transitions, latest_pack_per_phase: await latestPackPerPhase(q, feature.id), requirements: await requirementStatus(q, feature.id) };
}
